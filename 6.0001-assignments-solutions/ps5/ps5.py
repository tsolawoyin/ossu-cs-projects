...
class PhraseTrigger():
    ...
    def is_phrase_in(self, story):
        if not re.match("^([a-z]+\s?)+$", self._phrase.lower()):
            return False
        sentence =  story.lower()
        ...
        sentence = " ".join(filter(sentence))
        regex = r"\b" + f"{self._phrase}" + r"\b"

        return len(re.findall(regex, sentence)) != 0
...
class TitleTrigger(PhraseTrigger):
    ...
    def evaluate(self, story): # NewsStory will be passed in. And NewsStory is an object, so you will use the get_title method for this part
...
class TimeTrigger(Trigger):
    def __init__(self, time):
        ...
        # Makes sense. Learnt this stuff today. Cool
...
class BeforeTrigger(TimeTrigger):
    ...
    def evaluate(self, story):
        try:
            return self._time > story.get_pubdate()
        except:
            utc = pytz.UTC
            return utc.localize(self._time) > story.get_pubdate().replace(tzinfo=utc)
